...
```
This provides data in the form of a nested list. Each list is a generation. Each position in the list is an individual. The numerical value in the position represents the number of individual secondary cases. For example:
```{r}
set.seed(12345678)
# Example individual branching process with offspring distribution ~ NB(R,k) 
R <- 0.9
k <- 0.25
bp(offspring = rnbinom, size = k, mu = R)
```
In this example, generation `[[4]]` has 18 positions (the sum of generation `[[3]]`), representing 18 individuals. Twelve of them had 0 secondary cases, two had 1 secondary case, two had two secondary cases, one had 3 secondary cases, and one had 7 secondary cases. 
